...
import { generateUniqueKey, safeFormatTime, isClient } from '@/lib/utils';
...
const BasicChatComponent: React.FC<ChatProps> = ({
  currentUserId,
  receiverId,
  receiverName,
  receiverImage,
  ...
  // Load messages and start polling when chat room is ready
  useEffect(() => {
    if (chatRoomId) {
      fetchMessages();
      startPolling();
  ...
  }, [chatRoomId, fetchMessages, startPolling]);
    ...
    <div className="flex flex-col h-96 bg-white rounded-lg shadow-lg chat-container">
      ...
      <div className="flex items-center p-4 border-b border-gray-200 bg-blue-50 rounded-t-lg">
        ...
        <div className="flex items-center space-x-2">
          <div className="w-2 h-2 rounded-full bg-blue-500" title="Basic Mode (Polling)"></div>
          <span className="text-xs text-gray-500">Basic Mode</span>
          <button
