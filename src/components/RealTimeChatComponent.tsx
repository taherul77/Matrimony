...
const RealTimeChatComponent: React.FC<RealTimeChatProps> = ({
  currentUserId,
  receiverId,
  receiverName,
  receiverImage
    ...
    const trySocketIO = async () => {
      try {
        ...
        const connectionTimeout = setTimeout(() => {
          if (!socketConnected) {
            console.log('⏰ Socket.IO timeout - switching to polling');
            socket.disconnect();
            startPolling();
          }
        }, 5000);
        ...
        socket.on('disconnect', () => {
          console.log('❌ Socket.IO disconnected - switching to polling');
          setIsConnected(false);
          startPolling();
        });

        socket.on('connect_error', (error) => {
          console.log('❌ Socket.IO error - switching to polling:', error.message);
          clearTimeout(connectionTimeout);
          setIsConnected(false);
          startPolling();
        });
