...
const InterestsSentPage: React.FC = () => {
  ...
  const fetchSentInterests = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/interests?type=sent');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Interests Sent</h1>
          <p className="text-gray-600">Track the interests you've sent to other profiles</p>
        </div>
            ...
            <div key={interest.id} className="bg-white rounded-lg shadow-sm border p-6 hover:shadow-md transition-shadow">
              <div className="flex items-start space-x-4">
                ...
                <div className="relative">
                  <img
                    src={interest.receiverImage}
                    alt={interest.receiverName}
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center space-x-2">
                        <h3 className="text-lg font-semibold text-gray-900">{interest.receiverName}</h3>
                        <span className="text-gray-500">{interest.receiverAge} years</span>
                        {interest.isPremium && (
                      ...
                      <div className="flex items-center text-gray-600 text-sm mt-1">
                        ...
                        {interest.receiverLocation}
                      </div>
                      <div className="text-gray-600 text-sm">
                        <strong>Occupation:</strong> {interest.receiverOccupation}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200">
                    <div className="text-sm text-gray-500">
                      Sent on {new Date(interest.sentDate).toLocaleDateString()}
                    </div>
        ...
        {filteredInterests.length === 0 && (
          <div className="bg-white rounded-lg shadow-sm border p-12 text-center">
            ...
            <h3 className="text-xl font-medium text-gray-900 mb-2">
              {filter === 'all' ? 'No interests sent yet' : `No ${filter} interests`}
            </h3>
            <p className="text-gray-600 mb-6">
              {filter === 'all' 
                ? 'Start connecting with profiles that interest you'
...
export default InterestsSentPage;
